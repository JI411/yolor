...
def train(hyp, opt, device, tb_writer=None, wandb=None):
    ...
    # Logging
    if (wandb and wandb.run is None) or opt.sweep:
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        if rank in [-1, 0]:
            ...
            fi_ap50 = fitness_ap50(np.array(results).reshape(1, -1))  # weighted combination of [P, R, mAP@.3, mAP@.3-.8]
...
def parse_opt(known=False):
    ...
    parser.add_argument('--save-every-five', action='store_true', help='save model every five epoch')
